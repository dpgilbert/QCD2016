...
void QCDcheck::loop (const TChain * chain, const char* sample, const char* outdir, const int maxEvt) {
  ...
  TString h_mt2_title = Form("M_{T2} Distribution for #Delta#phi_{min} < 0.3;M_{T2} (GeV);Count @ %s fb^{-1}", lumi16);
  TH1F h_mt2("h_mt2", h_mt2_title.Data(), nmt2_bins, mt2_min, mt2_max);
  TString h_ht_title = Form("H_{T} Distribution for #Delta#phi_{min} < 0.3;H_{T} (GeV);Count @ %s fb^{-1}", lumi16);
  TH1F h_ht ("h_ht", h_ht_title.Data(), nht_bins, ht_min, ht_max);
  TString h_ht_mt2cut_title = Form("H_{T} Distribution for #Delta#phi_{min} < 0.3; H_{T} (GeV);Count @ %s fb^{-1}", lumi16);
  TH1F h_ht_mt2cut ("h_ht_mt2cut", h_ht_mt2cut_title.Data(), nht_bins, ht_min, ht_max);
  TString h_j1pt_title = Form("Jet 1 p_{T} Distribution for #Delta#phi_{min} < 0.3;Jet 1 p_{T} (GeV);Count @ %s fb^{-1}", lumi16);
  TH1F h_j1pt("h_j1pt", h_j1pt_title.Data(), njpt_bins, jpt_min, jpt_max);
  TString h_j2pt_title = Form("Jet 2 p_{T} Distribution for #Delta#phi_{min} < 0.3;Jet 2 p_{T} (GeV);Count @ %s fb^{-1}", lumi16);
  TH1F h_j2pt("h_j2pt", h_j2pt_title.Data(), njpt_bins, jpt_min, jpt_max);
  TString h_met_pt_title = Form("MET Distribution for #Delta#phi_{min} < 0.3;MET (GeV);Count @ %s fb^{-1}",lumi16);
  TH1F h_met_pt("h_met_pt", h_met_pt_title.Data(), nmet_bins, met_min, met_max);
  TString h_met_phi_title = Form("#phi_{MET} Distribution for #Delta#phi_{min} < 0.3;#phi_{MET};Count @ %s fb^{-1}", lumi16);
  TH1F h_met_phi("h_met_phi", h_met_phi_title.Data(), nphi_bins, phi_min, phi_max);
  TString h_jet1_phi_title = Form("Jet 1 #phi# Distribution;#phi_{Jet 1};Count @ %s fb^{-1}", lumi16);
  ...
  for (int event = 0; event < max_event; event++) {
    ...
    // QCD
    bool passQCDtrig = (mt2_tree.HLT_PFHT900 || mt2_tree.HLT_PFJet450 || mt2_tree.HLT_PFHT300_PFMET110 || mt2_tree.HLT_PFMET120_PFMHT120 || mt2_tree.HLT_PFMETNoMu120_PFMHTNoMu120) && (mt2_tree.met_miniaodPt / mt2_tree.met_caloPt) <= 5.0 && mt2_tree.nJet200MuFrac50DphiMet == 0;
